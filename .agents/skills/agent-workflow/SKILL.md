...
- **Skill projection.** APM 0.28 root-project integration deploys every `.apm/skills` bundle; `includes` and `--skill` do not filter root content. Do not add a local copy of a skill that the shared infrastructure package already provides; it would project a duplicate and be rejected. The local `plain-language` candidate was retired for this reason (D13).
