...
//!   - `jcode_tui::memory_graph::MemoryGraph` deserialization (real on-disk graphs)
//!   - `jcode_tui::embedding::embed` (real all-MiniLM-L6-v2 ONNX model)
//!   - `jcode_tui::memory::format_context_for_relevance` (real live query window)
...
use jcode_tui::embedding;
use jcode_tui::memory::format_context_for_relevance;
use jcode_tui::memory_graph::MemoryGraph;
...
fn cmd_queries(args: &[String]) -> Result<()> {
    ...
    for (path, _) in sessions {
        ...
        let user_turns: Vec<usize> = messages
            .iter()
            .enumerate()
            .filter(|(_, m)| matches!(m.role, jcode_tui::message::Role::User))
            .map(|(i, _)| i)
...
//
// The prompt/parse/rerank logic lives in the shared `jcode_tui::memory_rerank`
...
use jcode_tui::memory_rerank::{LLM_RERANK_SYSTEM, build_rerank_prompt, parse_rerank_response};
            ...
            .map(|input| {
                ...
                async move {
                    let sidecar = if backend == "openai" {
                        ...
                        jcode_tui::sidecar::Sidecar::with_openai_model(&model, eff)
                    } else {
                        jcode_tui::sidecar::Sidecar::with_claude_model(&model)
                    };
...
fn cmd_metrics(args: &[String]) -> Result<()> {
    ...
    let prod_mgr = if config == "prod_hybrid" {
        ...
        let mgr = jcode_tui::memory::MemoryManager::new().with_project_dir(project_dir);
    ...
    let alt_embedder = opts.get("embedder").map(|dir| {
        ...
        jcode_tui::embedding::Embedder::load_from_dir(Path::new(dir)).expect("load alt embedder")
    });
    ...
    let openai_backend = if config == "openai_dense" || config == "openai_hybrid" {
        use jcode_tui::embedding_backend::{
        ...
        let key = opts
            ...
            .or_else(|| {
                jcode_tui::provider_catalog::load_api_key(
                    &jcode_tui::provider_catalog::ApiKeyCredentialSource::primary_only(
                        "OPENAI_API_KEY",
                        "openai.env",
                    ),
                )
    ...
    let openai_corpus_emb: Vec<(String, Vec<f32>)> = match openai_backend.as_ref() {
        Some(b) => {
            use jcode_tui::embedding_backend::EmbeddingBackend;
    ...
    let ce_reranker = opts.get("reranker").map(|dir| {
        ...
        jcode_tui::embedding::CrossEncoder::load_from_dir(Path::new(dir)).expect("load reranker")
    });
        ...
        let raw: Vec<RecallResultRow> = rt.block_on(async {
            ...
            stream::iter(jobs)
                .map(|(qid, query, cands)| {
                    ...
                    async move {
                        let sidecar = if backend == "openai" {
                            jcode_tui::sidecar::Sidecar::with_openai_model(&model, None)
                        } else {
                            jcode_tui::sidecar::Sidecar::with_claude_model(&model)
                        };
                        let n = cands.len();
            ...
            stream::iter(jobs)
                .map(|(qid, query, cands)| {
                    ...
                    async move {
                        ...
                        let mut out_chars = 0usize;
                        for _ in 0..n_ens {
                            let sidecar = jcode_tui::sidecar::Sidecar::with_claude_model(&model);
                            for attempt in 0..2 {
                                match sidecar.complete(JUDGE_SYSTEM, &prompt).await {
                                    Ok(resp) => {
                                        out_chars += resp.len();
...
fn cmd_probe(args: &[String]) -> Result<()> {
    ...
    let e = jcode_tui::embedding::Embedder::load_from_dir(Path::new(&dir))?;
