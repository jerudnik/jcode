...
use jcode_tui::message::{ContentBlock, Role, ToolCall};
use jcode_tui::perf::{SyntheticSystemProfile, TuiPerfPolicy, tui_policy_for};
use jcode_tui::prompt::ContextInfo;
use jcode_tui::session::{Session, StoredDisplayRole};
use jcode_tui::side_panel::{
    SidePanelPage, SidePanelPageFormat, SidePanelPageSource, SidePanelSnapshot,
};
use jcode_tui::tui::{DisplayMessage, ProcessingStatus, TuiState, info_widget::InfoWidgetData};
...
struct BenchState {
    ...
    diff_mode: jcode_tui::config::DiffDisplayMode,
    queue_mode: bool,
    ...
    copy_selection_range: Option<jcode_tui::tui::CopySelectionRange>,
...
impl BenchState {
    fn new(
        ...
        Ok(Self {
            ...
            is_processing,
            status,
            diff_mode: jcode_tui::config::DiffDisplayMode::Off,
            queue_mode: true,
    ...
    fn from_session(
        ...
        let session = jcode_tui::replay::load_session(id_or_path)
            .with_context(|| format!("failed to load session '{}'", id_or_path))?;
        let mut side_panel =
            jcode_tui::side_panel::snapshot_for_session(&session.id).unwrap_or_default();
        ...
        Ok(Self {
            ...
            diff_mode: if matches!(mode, BenchMode::FileDiff) {
                jcode_tui::config::DiffDisplayMode::File
            } else {
                jcode_tui::config::DiffDisplayMode::Off
            },
            queue_mode: true,
            context_info: ContextInfo::default(),
            info_widget: InfoWidgetData::default(),
    ...
    fn simulate_linked_refresh(&mut self) -> Result<()> {
        ...
        let _ = jcode_tui::side_panel::refresh_linked_page_content(&mut self.side_panel, None);
        Ok(())
    }

    fn prewarm_side_panel(&self, width: u16, height: u16) -> bool {
        jcode_tui::tui::prewarm_focused_side_panel(
            &self.side_panel,
            width,
            height,
            40,
            jcode_tui::tui::mermaid::protocol_type().is_some(),
            false,
        )
    }
}
...
fn stored_message_visible_text(message: &jcode_tui::session::StoredMessage) -> String {
...
impl TuiState for BenchState {
    ...
    fn side_pane_images(&self) -> Vec<jcode_tui::session::RenderedImage> {
        Vec::new()
    }
    ...
    fn batch_progress(&self) -> Option<jcode_tui::bus::BatchProgress> {
        None
    }
    ...
    fn diff_mode(&self) -> jcode_tui::config::DiffDisplayMode {
        self.diff_mode
    }
    ...
    fn context_limit(&self) -> Option<usize> {
        Some(jcode_tui::provider::DEFAULT_CONTEXT_LIMIT)
    }
    ...
    fn render_streaming_markdown(&self, width: usize) -> Vec<ratatui::text::Line<'static>> {
        ...
        jcode_tui::tui::markdown::render_markdown_with_width(&self.streaming_text, Some(width))
    }
    ...
    fn auth_status(&self) -> jcode_tui::auth::AuthStatus {
        jcode_tui::auth::AuthStatus::default()
    }

    fn diagram_mode(&self) -> jcode_tui::config::DiagramDisplayMode {
        jcode_tui::config::DiagramDisplayMode::Pinned
    }
    ...
    fn diagram_pane_position(&self) -> jcode_tui::config::DiagramPanePosition {
        jcode_tui::config::DiagramPanePosition::default()
    }
    ...
    fn side_panel(&self) -> &jcode_tui::side_panel::SidePanelSnapshot {
        &self.side_panel
    }
    ...
    fn chat_native_scrollbar(&self) -> bool {
        jcode_tui::config::config().display.native_scrollbars.chat
    }

    fn side_panel_native_scrollbar(&self) -> bool {
        jcode_tui::config::config()
            .display
            .native_scrollbars
            .side_panel
    }
    ...
    fn inline_interactive_state(&self) -> Option<&jcode_tui::tui::InlineInteractiveState> {
        None
    }
    ...
    fn session_picker_overlay(
        &self,
    ) -> Option<&std::cell::RefCell<jcode_tui::tui::session_picker::SessionPicker>> {
        None
    }

    fn login_picker_overlay(
        &self,
    ) -> Option<&std::cell::RefCell<jcode_tui::tui::login_picker::LoginPicker>> {
        None
    }

    fn account_picker_overlay(
        &self,
    ) -> Option<&std::cell::RefCell<jcode_tui::tui::account_picker::AccountPicker>> {
        None
    }

    fn usage_overlay(
        &self,
    ) -> Option<&std::cell::RefCell<jcode_tui::tui::usage_overlay::UsageOverlay>> {
        None
    }
    ...
    fn copy_badge_ui(&self) -> jcode_tui::tui::CopyBadgeUiState {
        jcode_tui::tui::CopyBadgeUiState::default()
    }
    ...
    fn copy_selection_range(&self) -> Option<jcode_tui::tui::CopySelectionRange> {
        self.copy_selection_range
    }

    fn copy_selection_status(&self) -> Option<jcode_tui::tui::CopySelectionStatus> {
        None
    }
    ...
    fn cache_ttl_status(&self) -> Option<jcode_tui::tui::CacheTtlInfo> {
        None
    }
}
...
fn main() -> Result<()> {
    if std::env::var("JCODE_TUI_PROFILE").is_ok() {
        jcode_tui::logging::init();
        if let Some(path) = jcode_tui::logging::log_path() {
    ...
    if matches!(args.mode, BenchMode::MermaidFlicker) {
        let result = jcode_tui::tui::mermaid::debug_flicker_benchmark(args.frames.max(4));
    ...
    if matches!(args.mode, BenchMode::ImageScroll) {
        let result = jcode_tui::tui::mermaid::debug_image_scroll_benchmark(
            args.images,
            args.frames.max(4),
            args.images_visible,
        );
    ...
    if matches!(args.mode, BenchMode::FileDiff) {
        state.diff_mode = jcode_tui::config::DiffDisplayMode::File;
    }
    ...
    if profile_side_panel {
        jcode_tui::tui::mermaid::init_picker();
        jcode_tui::tui::mermaid::clear_active_diagrams();
        jcode_tui::tui::mermaid::clear_streaming_preview_diagram();
        jcode_tui::tui::clear_side_panel_render_caches();
        jcode_tui::tui::reset_side_panel_debug_stats();
        jcode_tui::tui::markdown::reset_debug_stats();
        jcode_tui::tui::mermaid::reset_debug_stats();
        if !args.keep_mermaid_cache {
            let _ = jcode_tui::tui::mermaid::clear_cache();
        }
    ...
    for frame in 0..args.frames {
        ...
        if matches!(args.mode, BenchMode::CopySelection) {
            ...
            state.copy_selection_range = Some(jcode_tui::tui::CopySelectionRange {
                start: jcode_tui::tui::CopySelectionPoint {
                    pane: jcode_tui::tui::CopySelectionPane::Chat,
                    abs_line: start_line,
                    column: 0,
                },
                end: jcode_tui::tui::CopySelectionPoint {
                    pane: jcode_tui::tui::CopySelectionPane::Chat,
                    abs_line: end_line,
                    column: usize::MAX / 4,
                },
        ...
        let markdown_before = profile_side_panel.then(jcode_tui::tui::markdown::debug_stats);
        let mermaid_before = profile_side_panel.then(jcode_tui::tui::mermaid::debug_stats);
        let side_panel_before = profile_side_panel.then(jcode_tui::tui::side_panel_debug_stats);
        let frame_start = Instant::now();
        terminal.draw(|f| jcode_tui::tui::render_frame(f, &state))?;
            ...
            && let Some(range) = state.copy_selection_range
        {
            ...
            if let Some(text) = jcode_tui::tui::debug_copy_selection_text_for_bench(range) {
        ...
        {
            let markdown_after = jcode_tui::tui::markdown::debug_stats();
            let mermaid_after = jcode_tui::tui::mermaid::debug_stats();
            let side_panel_after = jcode_tui::tui::side_panel_debug_stats();
    ...
    let side_panel_final_stats = profile_side_panel.then(jcode_tui::tui::side_panel_debug_stats);
    let markdown_final_stats = profile_side_panel.then(jcode_tui::tui::markdown::debug_stats);
    let mermaid_final_stats = profile_side_panel.then(jcode_tui::tui::mermaid::debug_stats);
    let mermaid_ui_summary = if profile_mermaid_ui {
        Some(summarize_mermaid_ui(
            &side_panel_profiles,
            jcode_tui::tui::mermaid::protocol_type().is_some(),
            jcode_tui::tui::mermaid::protocol_type().map(|p| format!("{:?}", p)),
        ))
    } else {
        None
    };
    let actual_policy = summarize_policy("detected", jcode_tui::perf::tui_policy());
    let synthetic_policy = args.synthetic_profile.map(|kind| {
        let synthetic = jcode_tui::perf::synthetic_profile(kind.to_system_profile());
        summarize_policy(
            kind.to_system_profile().label(),
            tui_policy_for(&synthetic, &jcode_tui::config::config().display),
        )
    });
