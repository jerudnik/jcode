...
";
    let out = redact_secrets(input);
    assert!(!out.contains("sk-ant-oat01-01234567890123456789"));
    assert!(!out.contains("sk-ant-ort01-01234567890123456789"));
    assert!(!out.contains("sk-or-v1-01234567890123456789"));
    assert!(!out.contains("ghp_01234567890123456789"));
    assert!(!out.contains("github_pat_01234567890123456789"));
    assert!(!out.contains("ya29.01234567890123456789"));
    assert!(!out.contains("AIza01234567890123456789"));
    assert!(!out.contains("xoxb-0123456789"));
    assert!(!out.contains("xoxp-0123456789"));
...
#[test]
fn redact_secrets_fast_path_skips_when_no_indicators() {
    let input = "This is a normal output from some command.\nIt has no keys.";
    let out = redact_secrets(input);
    assert_eq!(out, input);
}
...
fn redact_secrets_lowercase_indicators_without_matches_leave_output_unchanged() {
    ...
    // Contains "token" in lower case.
    let input = "The token parameter is missing.";
    let out = redact_secrets(input);
    assert_eq!(out, input);
}
