...
fn initial_session_context_uses_current_cwd_when_inserted() -> Result<()> {
    ...
    let _cwd_guard = super::CwdGuard::capture().map_err(|e| anyhow!(e))?;
    ...
    assert_eq!(
        ...
        Some(super::canonical_display(first_dir.path()).as_str())
    );
    ...
    assert!(
        first.contains(&format!(
            "Working directory: {}",
            super::canonical_display(second_dir.path())
        )),
        "session context should use cwd at insertion time, got: {first}"
    );
    assert_eq!(
        session.working_dir.as_deref(),
        Some(super::canonical_display(second_dir.path()).as_str())
    );
...
fn initial_session_context_can_refresh_before_real_conversation() -> Result<()> {
    ...
    assert!(session.messages[0].content_preview().contains(&format!(
        "Working directory: {}",
        super::canonical_display(first_dir.path())
    )));
