...
#[test]
fn test_file_activity_scroll_clears_trailing_ghost_after_native_scroll_like_mutation() {
    let _lock = scroll_render_test_lock();
    ...
    assert!(
        !scrolled.contains('Z'),
        "stale trailing ghost marker should be cleared after scroll-like repaint:\n{scrolled}"
    );
}
