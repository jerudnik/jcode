...
| **H3a** | HIGH | Hardening/migration pass index: the `jcode mcp-serve` fork-bomb incident and the follow-on self-reference guard, daemon/process caps, supervision hardening, and swarm comm migration now span incident docs, lifecycle invariants, and the patch ledger. | Keep this backlog as the pointer to "what changed / what's still open": see [MCP_SERVE_FORKBOMB_INCIDENT.md](MCP_SERVE_FORKBOMB_INCIDENT.md), [MCP_SERVER_REGISTRATION_GUARDRAILS.md](MCP_SERVER_REGISTRATION_GUARDRAILS.md), [SERVER_LIFECYCLE_INVARIANTS.md](../SERVER_LIFECYCLE_INVARIANTS.md), and [patch-ledger.md](../fork/patch-ledger.md). Still open: cap/guard-rejection fleet rows need a persisted side-channel because rejected spawns never become members. | doc index |
