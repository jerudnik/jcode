...
| Fork security policy: audit.toml + Security workflow | `distro(ci)` | `permanent-downstream` | none | Triaged advisory ignores live in `.cargo/audit.toml` (cargo-audit loads it natively); upstream-covered rationale lives in `docs/SECURITY_DEPENDENCIES.md`, and only genuinely fork-only rows belong in `docs/fork/SECURITY_TRIAGE.md`; gate + weekly full report live in `.github/workflows/security.yml`. Vendor `security_preflight.sh` ignore array stays pristine. | `cargo audit && actionlint .github/workflows/security.yml` |
