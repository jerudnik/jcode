...
| MCP self-reference guard (drop `jcode mcp-serve` shims) | `feature(fork-maint)` | `permanent-downstream` | none | Keep; daemon-owned MCP config now drops direct `jcode mcp-serve` registrations so the daemon cannot recursively launch its own MCP bridge. External MCP clients may still launch `jcode mcp-serve`. See `docs/architecture/MCP_SERVE_FORKBOMB_INCIDENT.md` and `docs/architecture/MCP_SERVER_REGISTRATION_GUARDRAILS.md`. | `cargo test -p jcode-base self_reference_guard_tests` |
| tester:spawn cap + depth guard | `feature(fork-maint)` | `permanent-downstream` | none | Keep; debug tester spawning is bounded by live tester count and a depth guard so tester daemons cannot recursively spawn more testers. | `cargo test -p jcode-app-core tester_spawn_policy_tests` |
| owned-MCP child cap (`OwnedChildPermit`) | `feature(fork-maint)` | `permanent-downstream` | none | Keep; owned MCP child processes reserve a permit before spawn and release it on drop, bounding daemon-owned MCP subprocess growth even if a guard is bypassed. | `cargo test -p jcode-base owned_child` |
| Headless session backstop (`MAX_TOTAL_SESSIONS`) | `feature(fork-maint)` | `permanent-downstream` | none | Keep; daemon headless session creation refuses new sessions at the total-session cap to backstop runaway auto-create paths. | `cargo test -p jcode-app-core headless_session_backstop_tests` |
| Always-on idle monitor decoupled from `JCODE_DEBUG_CONTROL` | `compat/feature(fork-maint)` | `permanent-downstream` | none | Keep; persistent-daemon idle exit runs independently of debug-control wiring so unattended daemons still retire after the idle window. | `cargo test -p jcode-app-core lifecycle` |
...
| Swarm dead-pid sweep skips terminal members | `perf` | `could-upstream` | none | Upstreamable cleanup; the dead-PID sweep skips members already in terminal states before touching disk, keeping long-lived swarm recovery bounded by non-terminal members. | `cargo test -p jcode-app-core swarm_dead_pid` |
| Channel primitive removal | `feature(fork-owned swarm/comm)` | `permanent-downstream` | none | Keep; removes `channel`, `subscribe_channel`, `unsubscribe_channel`, `channel_members`, and `list_channels` from the agent-facing surface. Matches the upstream `SWARM_TASK_GRAPH` §8a roadmap and is upstreamable as part of the DAG-first comm migration. | `cargo test -p jcode-app-core communicate channel` |
| Shared-context removal + `PlanProposalCache` | `feature(fork-owned swarm/comm)` | `permanent-downstream` | none | Keep; removes `share`, `share_append`, and `read` from generic comm and stores pending plan proposals in the dedicated in-memory `PlanProposalCache`. | `cargo test -p jcode-app-core comm_plan` |
| Fleet per-member data + cross-swarm stop | `feature(fork-owned swarm/comm)` | `permanent-downstream` | none | Keep; `SwarmFleetEntry.members`/`SwarmFleetMember` expose per-member session ids, `Request::CommStop` carries `cross_swarm`, and `/swarm stop` accepts `--cross-swarm`/`--force` for actionable fleet rows. | `cargo test -p jcode-protocol comm_requests && cargo test -p jcode-tui --lib fleet` |
| Workspace lib-test hermeticity | `feature(test-hygiene)` | `planned-upstream-pr` | none yet | Clean upstream-PR candidate: every fix isolates ambient state instead of changing product behavior. Retire individual fixes if/when upstream lands the same isolation. Environment-sensitive app-core tests share `crate::storage::lock_test_env()` and use temporary homes. Includes one real product fix (`JCODE_SHOW_AGENTGREP_OUTPUT` missing from `CONFIG_ENV_KEYS`). | `cargo test --workspace --lib --exclude jcode-tui` (Linux, clean `JCODE_HOME`) |
| Fork CI: macOS-first gate + tiered test execution | `distro(ci)` | `permanent-downstream` | none | See docs/BRANCHING.md "CI". fork-ci.yml gates main: macOS build + integration tests blocking, macOS lib tests advisory (promote after a clean week), Linux tests advisory on push / blocking weekly. Upstream ci.yml is dispatch-only and byte-close to the fork point. `jcode-tui` lib tests stay compile-only everywhere (see below). | `.github/workflows/fork-ci.yml`; `scripts/fork-health.sh` |
| Fork security policy: advisory ownership + Security workflow | `distro(ci)` | `permanent-downstream` | none | Keep. Suppression is spread across two surfaces that must agree: `.cargo/audit.toml` (cargo-audit loads it natively) and the vendor `audit_ignores` array in `scripts/security_preflight.sh`, which is what CI actually executes. Ownership metadata cannot live in either — cargo-audit validates `audit.toml` against a closed schema and rejects unknown keys, and the vendor script stays pristine — so it lives in `docs/security/advisories.toml`, one `[[advisory]]` per acceptance with owner, rationale, affected surface, expiry, and retirement condition. `scripts/check_advisory_policy.py` requires record↔surface agreement in both directions plus surface↔surface agreement, and fails on expiry. `docs/SECURITY_DEPENDENCIES.md` and `docs/fork/SECURITY_TRIAGE.md` are narrative only and enforce nothing (the pre-F22 check grepped them, which any passing mention satisfied). Gate + weekly full report live in `.github/workflows/security.yml`; `advisory-policy` must stay listed in `scripts/required-checks.json` **and** in the `evidence/R07` governance fixture. | `python3 scripts/check_advisory_policy.py && python3 -m unittest discover -s tests -p 'test_advisory_policy.py' && actionlint .github/workflows/security.yml` |
...
locally fetched upstream commit `802f6909825809e882d9c2d575b7e478dce57d3b`;
revalidate before citing these examples against a newer upstream ref:
