#!/usr/bin/env python3
"""R07 design.md section 4 maintenance window -- PR #132 (D02 expansion guard).
...
this program has already paid for:
  ...
  2. PINNED ENCODER. The canonical hash comes from the repository's own
     `scripts/governance_compare.py` `sanitize()`/`canonical()`, imported
     rather than reimplemented. Verified pre-write to agree with pr76's
     hand-rolled six-field form on the live body (both 43ba61a7a5...).
...
# Governance Root failed naming exactly the two protected paths below, and every
# other required context passed.
REVIEWED_HEAD = "9635f94bd88ae84478c975dde02392516e49c5f8"
...
sys.path.insert(0, f"{REPOROOT}/scripts")
from governance_compare import sanitize, canonical  # noqa: E402
...
def canon(obj):
    """Canonical hash via the repository's own pinned encoder."""
