...
A26 and A27 were added by the post-distribution graph amendment (D030) for work
identified after the closure audit; A16 and A23-A25 were reworded by the same
amendment to match the revised node contracts after the Nix-only distribution
transition.
...
| A16      | Structured, expiring security-advisory ownership policy                   | F22           | Mandatory deterministic      |
...
| A23      | Packaged browser control surface verified against a local gateway         | G03           | Mandatory deterministic      |
| A24      | Windows and FreeBSD not advertised as supported distribution surfaces     | G04           | Mandatory deterministic      |
| A25      | Disposable Nix/Cachix pinned-revision acquisition and launch smoke        | G05           | Authorization/network gated  |
| A26      | Active-documentation truth: claims, links, commands, instruction drift    | D01           | Mandatory deterministic      |
| A27      | Nix-only distribution and native-iOS retirement verification              | F30           | Mandatory deterministic      |
...
nodes that are not covered by at least one audit item. Coverage may additionally
cite D-prefix documentation nodes (A26).
