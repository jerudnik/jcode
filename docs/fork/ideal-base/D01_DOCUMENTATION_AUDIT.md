...
| `D01-F11` | low | D01-B | `TELEMETRY.md:283` cited the path "src/telemetry.rs", which does not exist. The implementation is `crates/jcode-telemetry-core/` (5 files, 3811 lines), reached through a `pub mod telemetry` re-export in `jcode-base`. Found while building F09's checker, not in the original census. | Correct the path. Verify the surrounding no-other-telemetry-calls claim rather than assuming only the path is wrong. |
...
| `D01-F12` | `corrected` | Reproduced by `scripts/measure_code_path_drift.py`: **84** stale code-path citations across **13** live files, plus **306** in two dated audit snapshots that are frozen by nature and not drift. Worst offenders `docs/WINDOWS.md` (18), `docs/MODULAR_ARCHITECTURE_RFC.md` (14), `docs/CRATE_OWNERSHIP_BOUNDARIES.md` (13). Not yet repaired and not yet gated: the measurement script is advisory only. Needs a ratcheted rule in `check_docs_references.py` starting at 84.  **Ratcheted at 86 and now enforced.** `check_docs_references.py` grew a `stale-code-path` rule: a backticked `crates|src|scripts|tests/**.rs|py|sh|nix` citation that `git ls-files` does not know. Ratcheted rather than fatal, because the debt is inherited and a fatal rule would just be permanently red. Dated audit snapshots are exempt: a document titled with its date is a record of a tree that has since moved, so flagging it would mean editing evidence to make a counter fall. **The two measurements disagreed, 84 vs 86, and the gap was resolved rather than averaged:** `measure_code_path_drift.py` never globbed `.audit/`, which is tracked and cites live code. The gate's wider scope was right and the script's glob was wrong, so the script was widened; both now read 86. Four controls run and observed: a new stale citation is rejected, `--update` refuses 18 -> 19, a resolving citation is not flagged, and fixing one lets the baseline fall to 85. **The ratchet then held me to its own rule:** control 4 left `docs/WINDOWS.md` pinned at 17, and since a ratchet cannot rise the only legal exit was to actually repair the citation, which is now `crates/jcode-base/src/platform.rs`. Four tests added, and the rule-flagging one was mutation-verified to fail when the rule is unwired. **Repaired to 0 on 2026-08-03, and the repair exposed a defect in the ratchet itself.** All 85 were classified by git history rather than by basename: a basename heuristic tried first mapped the path "src/tui/client.rs" to `crates/jcode-base/src/mcp/client.rs`, but that file was deleted in `c3184809e`, so it would have planted a false citation. 67 were renamed by the crates/ restructure and repointed; 11 were deleted, of which three frozen records of the retired upstream-tracking model were first exempted and then, on the same day, **un-exempted**: an exemption cannot be justified by a freeze nobody can guarantee, and measuring showed `CODE_PATH_EXEMPT` was hiding 359 citations across 6 files (not the 7 its rationale described), of which 42 resolved to live code and would rot silently while 317 were already stale. (An earlier version of this row said 32, which was the `PROVIDER_SESSION_SHARED_CONTRACT_AUDIT.md` baseline row transcribed as if it were the live-code count.) The constant is deleted; the per-file ratchet holds each frozen record at its inherited count, so it is never forced to falsify itself while anything added beyond that count still fails. Mutation-verified: restoring the exemption makes a planted citation go silent and fails exactly the two new tests; 7 were never tracked as written, and one of those, `jade_relay.rs`, turned out to exist under `crates/jcode-app-core`, so my own NEVER label was wrong and was corrected. The rest are illustrative or forward-looking paths in plan documents, unbacktick-ed so they stop rendering as citations you can open. `MODULAR_ARCHITECTURE_RFC.md` needed more than paths: its "current chokepoints" line counts were stale and "the root crate still has" was false, since the split it proposes has largely happened. **The defect:** a rule driven to zero has an empty per-file dict, which `write_baselines` read as "never measured", so `--update` would have accepted any regression as a first measurement. Completing the cleanup would have disarmed the gate. Rule state is now keyed off the `<key>_total` the baseline already records, with a regression test that fails when the guard is reverted. |
