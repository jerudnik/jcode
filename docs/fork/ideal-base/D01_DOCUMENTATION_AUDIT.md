...
| `D01-F14` | medium | product (not D01) | **Product defect, found while documenting F05.** Three dangling wires make three complete ambient implementations inert. (1) `UsageLog::record` (`ambient/scheduler.rs:26`) has no non-test caller, so `usage.json` is never written and every rate query returns 0. (2) `RateLimitInfo` has no production constructor and no `x-ratelimit` header is parsed anywhere, so both `calculate_interval` call sites (`ambient/runner.rs:686,819`) pass `None` and the adaptive scheduler always returns the max interval. (3) `active_user_sessions` (`ambient/runner.rs:53`) has reads at :176, :585 and :879 but **no writer in the workspace**, so it is permanently 0 and ambient never pauses for an active user. The budget bar is inert downstream of (1) and (2): both producers hardcode `budget_percent: None`. | Not a documentation repair. `docs/AMBIENT_MODE.md` marks each row Inert with evidence. Each fix is a small connection, not a feature build. Needs a separately owned product node. |
