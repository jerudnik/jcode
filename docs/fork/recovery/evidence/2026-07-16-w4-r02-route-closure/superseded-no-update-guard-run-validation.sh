...
run_expect no_update_guard 0 /usr/bin/python3 -c 'import pathlib,sys; root=pathlib.Path(sys.argv[1]); needle="-"+"-update"; bad=[]
for p in list((root/"raw").glob("*.txt"))+[root/"manifest.tsv"]:
    if p.name=="no_update_guard.txt": continue
    if needle in p.read_text(errors="replace"):
        bad.append(str(p))
print("NO_UPDATE_HITS", len(bad))
print("\n".join(bad))
