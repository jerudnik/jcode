...
out="$repo/docs/fork/recovery/evidence/2026-07-16-w5-onboarding-consent/correction-run"
mkdir -p "$out"
cd "$repo"
...
JCODE_HOME=$(mktemp -d /tmp/jcode-w5-correction-home.XXXXXX)
JCODE_RUNTIME_DIR=$(mktemp -d /tmp/jcode-w5-correction-runtime.XXXXXX)
cleanup() {
...
echo "correction_for=dfe5d1ec4b359ea68d956eab9feaa62399e29618" >>"$out/run.meta"
...
echo 'JCODE_CHECKPOINT {"message":"W5 onboarding consent correction-run evidence passed all expected exits"}'
