...
run_expect process_before 0 bash -lc "ps -axo pid,ppid,comm,args | awk '/nix-daemon __build-remote|ssh john@10\\.201\\.0\\.7|ssh: \/tmp\/nix/ && \\$0 !~ /awk/ {print}'"
...
run_expect prompt_hash 0 bash -lc 'test "$(git diff -- docs/fork/recovery/ORCHESTRATOR_PROMPT.md | shasum -a 256 | awk "{print \\$1}")" = 8e8e6a92dad180b3925bc0b2a3b7b951bc6a6f5c9e4f8a57c9f522d03ad85c00'
run_expect stash_count 0 bash -lc 'test "$(git stash list | wc -l | tr -d " ")" = 4'
run_expect process_after 0 bash -lc "ps -axo pid,ppid,comm,args | awk '/nix-daemon __build-remote|ssh john@10\\.201\\.0\\.7|ssh: \/tmp\/nix/ && \\$0 !~ /awk/ {print}'"
run_expect no_update_invocation 0 /usr/bin/python3 -c 'import pathlib,sys; root=pathlib.Path(sys.argv[1]); needle="-"+"-update"; bad=[]
for p in [root/"manifest.tsv", pathlib.Path("/tmp/jcode-w4-integration-driver.sh")]:
