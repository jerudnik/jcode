...
- Revision: v2, 2026-07-15. Two bounded coordinator-audit corrections applied to v1 (which is preserved unchanged at `/tmp/jcode-phase4-recovery-plan-fable.md`): (a) disposition arithmetic corrected to fourteen retain-fork / two compose / one defer, since the R04 ledger recommends `retain-fork`; (b) W1 acceptance restated to the R12 invariant of one correlated terminal response per emitted provider request, permitting multiple requests on retry paths, rather than overstating exactly-one-request for retry fixtures.
