# Proposed RECOVERY_PLAN.md (Phase 4 Cross-Seam Architecture)
...
- Status: **PROPOSED PASS**, pending coordinator audit and fixed-plan independent approval. This document authorizes nothing by itself. No spot-checker child was dispatched; every decisive fact cited below was reproduced locally with read-only Git/hash/source commands or is quoted from a hash-anchored ledger.
...
3. **Seam-by-seam authority is overwhelmingly fork-side.** Thirteen of seventeen integrated ledgers conclude `retain-fork` (R00, R01, R03A, R03B, R05A, R05B, R06A, R07A, R07C, R09, R11, R12, R13). Two conclude `compose` with narrowly named upstream candidates (R02 tier/catalog; R10 draft-release finalization). One is `defer` with escalation (R08A). Upstream has literally no counterpart for the fork's evidence spine (R06A/R12), control log (R05A), handshake (R03A), reload-target policy (R01 `paths.rs` upstream delta is empty), or Nix packaging (R10 checkpoint 2). There is nothing to replay *onto*.
...
- **Acceptance:** deterministic fixtures for before-open cancel, mid-stream cancel, and open/mid-stream context-limit retry each show exactly one request, one correlated non-`Ok` response, one non-`Ok` finish; the five existing `r12_*` fixtures stay green; no duplicate response path introduced (the ledger's negative finding must survive).
