...
pub(super) fn build_cut_release_prompt() -> String {
    ...
    prompt.push(' ');
    prompt.push_str(
        "Then cut a release by following RELEASING.md. Find the last release tag (git describe --tags --abbrev=0 or gh release list) and review everything that changed since it to pick the semver bump: patch for fixes and small internal changes, minor for new features, major only for breaking changes. Bump the version in the root Cargo.toml, refresh Cargo.lock, and, if the repo has a changelog/ directory, write a user-facing changelog entry changelog/v<version>.json following changelog/README.md (translate commits into user-visible effects, skip internal-only changes, update changelog/index.json). Run the required Rust, Nix-distribution-policy, workflow, documentation, and flake checks. Commit and push the release preparation, ensure that exact commit is on authoritative main, then create and push the immutable v<version> tag. The tag workflows must publish the flake outputs to Cachix and a metadata-only GitHub release with zero executable assets. Never recreate installers, Homebrew/AUR publication, archive/checksum assets, or Cargo registry publication. Do not force-push or move existing tags. Finally, report the new version, commits, tag, Nix/Cachix result, and metadata-release result.",
    );
    prompt
}
