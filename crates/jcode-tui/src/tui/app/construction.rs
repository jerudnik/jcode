...
fn current_binary_mtime() -> Option<std::time::SystemTime> {
    let path = match std::env::current_exe() {
        Ok(path) => path,
        Err(error) => {
            crate::logging::debug(&format!("Current executable path unavailable: {error}"));
            return None;
        }
    };
    let metadata = match std::fs::metadata(&path) {
        ...
        Err(error) => {
            crate::logging::debug(&format!("Current executable metadata unavailable: {error}"));
            return None;
        }
    };
    match metadata.modified() {
        ...
        Err(error) => {
            crate::logging::debug(&format!(
                "Current executable modification time unavailable: {error}"
            ));
            None
        }
    }
...
impl App {
    pub(super) fn new_minimal_with_session(
        ...
        let mut app = Self {
            ...
            auto_poke_followups_sent: 0,
            last_auto_poke_fingerprint: None,
            todo_confidence_gate_fired_for: None,
            ...
            remote_history_recovery: Default::default(),
            ...
            client_focused: true,
            terminal_modes: crate::tui::TerminalModeState::default(),
            runtime_memory_log,
            idle_heap_release: Default::default(),
            client_binary_mtime: current_binary_mtime(),
    ...
    fn new_with_session_activity(
        ...
        if let Ok(handle) = tokio::runtime::Handle::try_current() {
            ...
            handle.spawn(async move {
                if let Err(error) = provider_clone.prefetch_models().await {
                    crate::logging::debug(&format!("Provider model prefetch failed: {error}"));
                }
            });
        }
    ...
    fn restore_remote_startup_history(&mut self, session_id: &str) {
        ...
        let image_ms = 0;
        let side_panel_snapshot = match crate::side_panel::snapshot_for_session(session_id) {
            Ok(snapshot) => snapshot,
            Err(error) => {
                crate::logging::debug(&format!(
                    "Remote startup side-panel restore unavailable for {session_id}: {error}"
                ));
                crate::side_panel::SidePanelSnapshot::default()
            }
        };
        self.set_side_panel_snapshot(side_panel_snapshot);
    ...
    pub fn new_for_remote_with_options(resume_session: Option<String>, fresh_spawn: bool) -> Self {
        ...
        #[cfg(test)]
        let test_env_lock = super::test_support::test_app_env_lease();
        ...
        let session = resume_session
            ...
            .and_then(|session_id| match Session::load_startup_stub(session_id) {
                ...
                Err(error) => {
                    crate::logging::debug(&format!(
                        "Remote startup stub unavailable for {session_id}: {error}"
                    ));
                    None
                }
            })
