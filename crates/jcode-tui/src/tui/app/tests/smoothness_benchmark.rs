...
fn smoothness_benchmark_simulated_streaming_turn_stays_within_budget() {
    // Keep the developer's ambient provider keys out of this benchmark: an
    // extra provider badge row changes the messages-area height and the
    // block-pop pattern the budgets below were calibrated against.
