use super::{
    AmbientConfig, Config, ConfigProvenance, DiffDisplayMode, DisplayConfig, LatexRenderingMode,
...
pub(super) fn restore_env_var(key: &str, previous: Option<OsString>) {
...
fn global_config_cache_reloads_after_manual_file_edit() {
    ...
    let _ = crate::config::take_config_reload_reasons();
    for _ in 0..1_000 {
        assert!(!crate::config::config().display.centered);
    }
    assert_eq!(
        ...
        initial_generation,
        "repeated cached reads must not reparse an unchanged config; reload reasons: {:?}",
        crate::config::take_config_reload_reasons()
    );
    ...
    assert_eq!(
        ...
        "one fingerprint change must trigger exactly one cache reload; reload reasons: {:?}",
        crate::config::take_config_reload_reasons()
    );
