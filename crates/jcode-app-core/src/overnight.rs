...
pub(crate) async fn run_supervisor(
    ...
    // An overnight run is unattended by construction: it is scheduled to run
    // while the user is asleep. Register for the whole supervisor loop, not per
    // turn, so every turn is gated; the guard unregisters on drop, including
    // the `?` returns inside the loop below.
    let _ambient_guard = AmbientSessionGuard::new(child_session_id);
    record_event(
        &manifest,
        "preflight_started",
...
/// Build a manifest whose wake and grace windows have already elapsed, so a
/// supervisor run takes exactly one coordinator turn and then completes.
///
/// Lives in the production module (behind `cfg(test)`) rather than in the
/// consuming test file because `OvernightManifest`'s fields are private to this
/// module's constructors.
#[cfg(test)]
pub(crate) fn test_manifest_past_wake(
    root: &Path,
    coordinator_session_id: &str,
) -> OvernightManifest {
    let run_dir = root.join("overnight_run");
    std::fs::create_dir_all(&run_dir).expect("run dir");
    let now = Utc::now();
    OvernightManifest {
        version: OVERNIGHT_VERSION,
        run_id: "overnight_gate_probe".to_string(),
        parent_session_id: "parent".to_string(),
        coordinator_session_id: coordinator_session_id.to_string(),
        coordinator_session_name: "coordinator".to_string(),
        started_at: now - ChronoDuration::hours(10),
        target_wake_at: now - ChronoDuration::hours(3),
        handoff_ready_at: now - ChronoDuration::hours(4),
        post_wake_grace_until: now - ChronoDuration::hours(1),
        morning_report_posted_at: Some(now - ChronoDuration::hours(2)),
        completed_at: None,
        cancel_requested_at: None,
        status: OvernightRunStatus::Running,
        mission: Some("gate probe".to_string()),
