...
mod tests {
    ...
    #[test]
    fn structured_stream_read_error_with_generic_message_is_transient() {
        // The default fallback text contains "stream error", which would mask this code arm.
        let error = serde_json::json!({
            "type": "upstream_error",
            "code": "stream_read_error",
        ...
        assert_eq!(
            message,
            "upstream_error (stream_read_error): provider failed"
        );
        assert_eq!(retry_after, None);
        assert!(jcode_provider_core::is_transient_transport_error(&message));
    }
