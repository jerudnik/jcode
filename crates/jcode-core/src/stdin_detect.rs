...
mod macos {
    ...
    // Keep the fd-detail libproc bindings colocated with the active fd-listing
    // probe. They are macOS FFI constants/functions reserved for deeper stdin
    // diagnostics and may be unused on current builds.
    #[allow(dead_code)]
    unsafe extern "C" {
        fn proc_pidinfo(
            pid: i32,
